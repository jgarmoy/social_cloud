...
renamed as (
    select
        ...
        case 
            when tk_handle ilike '@%' then substr(trim(tk_handle), 2)
            else tk_handle
        end as tk_handle,
        case
            when tk_platform is not null then 'Tiktok'
            else tk_platform
        end as tk_platform,
        snapshot_date,
        ...
        seg_followers,
        case 
            when ig_username ilike '@%' then substr(trim(ig_username), 2)
            else ig_username
        end as ig_username,
        case
            when ig_platform is not null then 'Instagram'
            else ig_platform
        end as ig_platform,
        report_date,
        ...
        follower_count_seg,
        case 
            when yt_handle ilike '@%' then substr(trim(yt_handle), 2)
            else yt_handle
        end as yt_handle,
        case
            when network is not null then 'Youtube'
            else network
        end as network,
        period,
